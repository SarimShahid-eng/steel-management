@extends('partials.app', ['title' => 'Customer Payment'])
    ...
    <div class="col-span-12 space-y-6 xl:col-span-12">
        ...
        <div class="flex flex-col gap-3 mb-6 sm:flex-row sm:items-center sm:justify-between px-3">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Manage customer payments</p>
            </div>
            ...
            <div class="mb-4 flex flex-col gap-3 px-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                <div>
                    ...
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">
                        {{ $customerPayments->total() }} customer payments{{ $customerPayments->total() !== 1 ? 's' : '' }}
                        found
                    </p>
                </div>

                {{-- SEARCH --}}
                <form method="GET" action="{{ route('customerPayment.index') }}">
                ...
                <table class="min-w-full">
                    ...
                    <thead class="border-y border-gray-100 py-3">
                        <tr>
                            ...
                            <th class="py-3 px-4 text-left text-theme-sm text-gray-500">Customer Account</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        @forelse($customerPayments as $index => $customerPayment)
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">

                                <td class="py-3 pr-4 text-gray-500 text-sm">
                                    {{ $customerPayments->firstItem() + $index }}
                                </td>

                                <td class="py-3 px-4">
                                    <span class="font-mono text-sm font-medium text-gray-800 dark:text-white/90">
                                        {{ @$customerPayment->transaction->voucher }}
                                    </span>
                                </td>

                                <td class="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">
                                    {{ \Carbon\Carbon::parse($customerPayment->date)->format('d M Y') }}
                                </td>

                                <td class="py-3 px-4 text-sm font-medium text-gray-800 dark:text-white/90">
                                    {{ $customerPayment->customerAccounts->name }}
                                </td>

                                <td class="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">
                                    {{ $customerPayment->type ?? '—' }}/
                                    {{ $customerPayment->paymentAccounts->name }}
                                </td>
                                ...
                                <td class="py-3 px-4 text-sm text-right font-medium text-error-500">
                                    {{ number_format($customerPayment->amount, 0) }}
                                </td>

                                {{-- <td class="px-5 py-4 text-center">
                                    <div class="inline-flex items-center gap-1">
                                        <a href="{{ route('customerPayments.show', $supplier->id) }}" title="View"
                                ...
                                <td>
                                    <div class="inline-flex items-center gap-1">
                                        <a href="{{ route('customerPayment.edit', $customerPayment->id) }}" title="Edit"
                                            class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-gray-500 hover:text-brand-500 hover:bg-brand-50 dark:text-gray-400 dark:hover:text-brand-400 dark:hover:bg-brand-500/10 transition-colors">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="py-16 text-center text-gray-500">
                                    No customers payment entry found
                                </td>
                            </tr>
                        @endforelse
