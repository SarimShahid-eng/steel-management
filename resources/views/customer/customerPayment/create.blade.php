@extends('partials.app', ['title' => 'Create Customer Payment'])
    ...
    <div class="col-span-12 space-y-6 xl:col-span-12">
        ...
        <div class="p-4 mx-auto max-w-screen-2xl md:p-6">
            ...
            <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="text-title-sm font-semibold text-gray-800 dark:text-white/90">New Customer Payment</h2>
                    <p class="mt-0.5 text-theme-sm text-gray-500 dark:text-gray-400">Record an customer payment and update
            ...
            <form method="POST"
                action="{{ isset($customerPayment) ? route('customerPayment.update', $customerPayment->id) : route('customerPayment.store') }}"
                {{-- action="{{ route('customerPayment.store') }}" --}} x-data="customerForm()">
                @csrf
                ...
                <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] mb-5">
                    ...
                    <div class="px-5 py-4 sm:px-6 sm:py-5">
                        <h3 class="text-base font-medium text-gray-800 dark:text-white/90">Customer Payment Details</h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Select the customer account, payment
                        ...
                        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                            ...
                            <div>
                                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Customer Payment Account <span class="text-error-500">*</span>
                                </label>
                                <div class="relative">
                                    <select name="account_id" x-model="customerAccountId" @class([
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="number" name="amount"
                                        value="{{ old('amount', @$customerPayment->amount) }}" min="1" step="1"
                                        placeholder="0" @class([
                            ...
                            <input type="hidden" name="update_id" value="{{ @$customerPayment->id }}">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="date" name="date"
                                        value="{{ old('date', isset($customerPayment) ? $customerPayment->date : now()->format('Y-m-d')) }}"
                                        @class([
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="description" rows="2" placeholder="e.g. Monthly rent payment, fuel for delivery"
                                    class="dark:bg-dark-900 shadow-theme-xs w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden focus:border-brand-300 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('description', @$customerPayment->description) }}</textarea>
                            </div>
                ...
                <div class="rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-white/[0.03] mb-5 overflow-hidden"
                    x-show="customerAccountId && paymentType">
                    ...
                    <div class="divide-y divide-gray-100 dark:divide-gray-800">
                        <div class="flex items-center justify-between px-5 py-3">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-gray-700 dark:text-gray-300">Customer Payment account</span>
                            </div>
    ...
    @push('scripts')
        <script>
            function customerForm() {
                return {
                    customerAccountId: '{{ old('account_id', $customerPayment->account_id ?? '') }}',
                    paymentType: '{{ old('payment_type', $customerPayment->type ?? '') }}',
                    paymentAccountId: '{{ old('payment_account_id', $customerPayment->payment_account_id ?? '') }}',
                    paymentAccounts: [],
