@extends('partials.app', ['title' => 'Create Bank or Cash'])
    ...
    <div class="col-span-12 space-y-6 xl:col-span-12">
        <div class="p-4 mx-auto max-w-screen-2xl md:p-6">
            ...
            <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="text-title-sm font-semibold text-gray-800 dark:text-white/90">Add Bank Or Cash</h2>
                    <p class="mt-0.5 text-theme-sm text-gray-500 dark:text-gray-400">Fill in the details to create a new
                        Bank Or Cash Account</p>
                </div>
                <a href="{{ route('bankCash.index') }}"
                    ...
                    Back to Bank or Cashs
                </a>
            </div>

            <form method="POST" action="{{ route('bankCash.store') }}">
                @csrf
                <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">

                    {{-- Card Header --}}
                    <div class="px-5 py-4 sm:px-6 sm:py-5">
                        <h3 class="text-base font-medium text-gray-800 dark:text-white/90">Bank or Cash Information</h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Basic details information for the Bank or
                            Cash.
                        </p>
                    </div>
                    <input type="hidden" name="update_id" value="{{ @$bankCash->id }}">
                        ...
                        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select name="type" id="type"
                                    class="shadow-theme-xs rounded-lg w-full border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden focus:border-brand-300 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                                    <option value="bank" @selected(@$bankCash->type === 'bank')>
                                        Bank
                                    </option>
                                    <option value="cash" @selected(@$bankCash->type === 'cash')>
                                        Cash
                                    </option>
                                </select>
                            </div>
                            <div>
                                {{--cash 5425.00 --}}
                                {{--supplier 2543.00 --}}
                                <x-input-text name="opening_balance"
                                disabled="{{ isset($bankCash) ? true : false }}"
                                type="number" label="Opening Balance"
                                    :value="@$bankCash->opening_balance" placeholder="100-200000000..." required />
                            </div>
