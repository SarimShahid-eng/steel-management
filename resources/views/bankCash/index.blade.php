@extends('partials.app', ['title' => 'Bank and Cash Accounts'])
...
@section('content')
    <div class="col-span-12 space-y-6 xl:col-span-12">
        ...
        <div class="flex flex-col gap-3 mb-6 sm:flex-row sm:items-center sm:justify-between px-3">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Bank or Cash</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Manage Bank/Cash</p>
            </div>

            <a href="{{ route('bankCash.create') }}"
        ...
        <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="mb-4 flex flex-col gap-3 px-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                <div>
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">All Bank/cash</h3>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">
                        {{ $banksCashes->total() }} bank or cash{{ $banksCashes->total() !== 1 ? 's' : '' }} found
                    </p>
                </div>

                {{-- SEARCH --}}
                <form method="GET" action="{{ route('bankCash.index') }}">
                    <div class="flex items-center gap-2">

                        {{-- <x-search-input placeholder="Search bankCash..." />
                        ...
                        @endphp
                        <x-filter-toolbar :dateRange="false" :select-options="$selectOptions" select-name="type" select-id="customerSelect"
            ...
            <div class="custom-scrollbar max-w-full overflow-x-auto px-5 sm:px-6">
                <table class="min-w-full">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        @forelse($banksCashes as $index => $bankCash)
                            <tr class="hover:bg-gray-50 transition-colors">

                                <td class="py-3 pr-4">
                                    {{ $banksCashes->firstItem() + $index }}
                                </td>
                                ...
                                <td class="px-5 py-4 text-center">
                                    <div class="inline-flex items-center gap-1">
                                        <a href="{{ route('bankCash.edit', $bankCash->id) }}" title="Edit"
