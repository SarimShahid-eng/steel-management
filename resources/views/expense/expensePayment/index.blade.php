@extends('partials.app', ['title' => 'Expense Payment'])
...
@section('content')
    <div class="col-span-12 space-y-6 xl:col-span-12">
        ...
        <div class="flex flex-col gap-3 mb-6 sm:flex-row sm:items-center sm:justify-between px-3">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Expenses</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Manage expense payments</p>
            </div>

            <a href="{{ route('expensePayment.create') }}"
        ...
        <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="mb-4 flex flex-col gap-3 px-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                <div>
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">All Expenses</h3>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">
                        {{ $expenses->total() }} expense{{ $expenses->total() !== 1 ? 's' : '' }} found
                    </p>
                </div>

                {{-- SEARCH --}}
                <form method="GET" action="{{ route('expensePayment.index') }}">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                                ...
                                <td class="py-3 px-4">
                                    <span class="font-mono text-sm font-medium text-gray-800 dark:text-white/90">
                                        EV-{{ $expense->transaction->voucher_no }}
                                    </span>
                                </td>
                                ...
                                <td class="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">
                                    {{ $expense->paymentFrom->type ?? '—' }}/
                                    {{ $expense->paymentFrom->name ?? '—' }}
                                </td>
                                ...
                                {{-- <td class="px-5 py-4 text-center">
                                    <div class="inline-flex items-center gap-1">
                                        <a href="{{ route('expenses.show', $expense->id) }}" title="View"
