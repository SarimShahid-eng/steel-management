{{-- ═══════════════════════════════════════════════════════════ --}}
{{-- SALE ITEMS MODAL                                            --}}
{{-- Usage: add x-data="saleModal()" to your table wrapper      --}}
{{-- Add @click="open({{ $sale->id }})" to any trigger          --}}
{{-- ═══════════════════════════════════════════════════════════ --}}

{{-- SALE MODAL --}}
<div x-show="isOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center ml-[180px]"
            ...
            <div>
                <h3 class="text-base font-medium text-gray-800 dark:text-white/90">
                    ...
                    <span class="font-mono text-sm text-brand-500 ml-1"
                        x-text="voucherNo ? 'SV-' + voucherNo : ''"></span>
                </h3>
                <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400" x-text="customerName"></p>
            </div>
            ...
            <div
                ...
                <div class="px-5 py-3">
                    ...
                    <p class="mt-0.5 text-sm font-semibold text-green-600 dark:text-green-400"
                        x-text="formatNum(receivedAmount)"></p>
                </div>
                <div class="px-5 py-3">
        ...
        function saleModal() {
            return {
                ...
                async open(saleId) {
                    ...
                    this.voucherNo = null;
                    this.customerName = '';
                    const url = "{{ route('sale.fetchProductDetails', ':sale') }}".replace(':sale', saleId);
                    try {
