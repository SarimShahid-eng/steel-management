...
<form method="GET" action="{{ $action }}" class="flex flex-wrap items-center gap-2">
    ...
    @endif
    {{-- @dd($singleDate) --}}
    @if ($singleDate && !is_null($dateName))
    {{-- @dd('s') --}}
        <x-date-input name="{{ $dateName }}" value="{{ request($dateName, '') }}"/>
    @endif
...
{{-- @dd($dateName) --}}
    @if (request()->anyFilled(array_filter([$selectName,$dateName, 'date_range', 'search'])))
        <a href="{{ $action }}"
