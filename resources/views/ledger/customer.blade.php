...
@section('content')
    <div class="col-span-12 space-y-6 xl:col-span-12">
        ...
        <div class="flex flex-col gap-3 mb-6 sm:flex-row sm:items-center sm:justify-between px-3">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Customer Ledger</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Transaction history and balance per customer</p>
            </div>
        </div>

        {{-- FILTERS --}}
        <form method="GET" action="{{ route('ledger.customer') }}">
                        ...
                        <div class="relative">
                            <select name="account_id"
                                ...
                                @foreach ($customers as $customer)
                                    <option value="{{ $customer->id }}" {{ request('account_id') == $customer->id ? 'selected' : '' }}>
                                        {{ $customer->name }}
                                    </option>
                                @endforeach
                            </select>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="mb-4 px-5 sm:px-6">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">
                    {{ $account ? $account->name : 'All Customers' }} — Transaction History
                </h3>
            ...
            <div class="custom-scrollbar max-w-full overflow-x-auto px-5 sm:px-6">
                <table class="min-w-full">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="py-16 text-center text-gray-500">No transactions found</td>
                            </tr>
                        @endforelse
