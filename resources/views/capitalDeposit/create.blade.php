@extends('partials.app', ['title' => 'Create Capital/Deposit'])
...
@section('content')
    ...
    <div class="col-span-12 space-y-6 xl:col-span-12">
        <div class="p-4 mx-auto max-w-screen-2xl md:p-6">
            ...
            <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="text-title-sm font-semibold text-gray-800 dark:text-white/90">New Capital/Deposit</h2>
                    <p class="mt-0.5 text-theme-sm text-gray-500 dark:text-gray-400">Record an Capital/Deposit payment and update
                ...
                <a href="{{ route('capitalDeposit.index') }}"
                    ...
                    Back to Capital/Deposits
                </a>
            </div>

            <form method="POST" action="{{ route('capitalDeposit.store') }}" x-data="capitalDepositForm()">
                @csrf

                {{-- ── SECTION 1: Capital/Deposit Details ── --}}
                <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] mb-5">

                    <div class="px-5 py-4 sm:px-6 sm:py-5">
                        <h3 class="text-base font-medium text-gray-800 dark:text-white/90">Capital/Deposit Details</h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Select the Capital/Deposit account, payment
                        ...
                        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                            ...
                            {{-- <div>
                                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Capital/Deposit Account <span class="text-error-500">*</span>
                                </label>
                                <div class="relative">
                                    <select name="account_id" x-model="capitalAccountId"
                                        @class([
                                            ...
                                            'border-error-300 focus:border-error-300 focus:ring-error-500/10 dark:border-error-700' => $errors->has('account_id'),
                                            'border-gray-300 focus:border-brand-300 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900' => !$errors->has('account_id'),
                                        ])>
                                        <option value="" disabled selected>Select bankCash account</option>
                                        @foreach ($bankOrCashAccounts as $account)
                            ...
                            <div>
                                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Deposit In <span class="text-error-500">*</span>
                                </label>
                                <div class="relative">
                                    <select name="payment_type" x-model="paymentType"
                                        @class([
                                            'dark:bg-dark-900 shadow-theme-xs w-full rounded-lg border bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:text-white/90 appearance-none',
                                            'border-error-300 focus:border-error-300 focus:ring-error-500/10 dark:border-error-700' => $errors->has('payment_type'),
                                            'border-gray-300 focus:border-brand-300 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900' => !$errors->has('payment_type'),
                                        ])>
                                ...
                                <div class="relative">
                                    <select name="payment_account_id" x-model="paymentAccountId"
                                        ...
                                        @class([
                                            ...
                                            'border-error-300 focus:border-error-300 focus:ring-error-500/10 dark:border-error-700' => $errors->has('payment_account_id'),
                                            'border-gray-300 focus:border-brand-300 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900' => !$errors->has('payment_account_id'),
                                        ])>
                                    ...
                                    <span class="pointer-events-none absolute top-1/2 right-3.5 -translate-y-1/2 text-gray-400">
                                        <svg x-show="accountsLoading" class="animate-spin" width="16" height="16" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3"/>
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/>
                                        </svg>
                                        <svg x-show="!accountsLoading" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <polyline points="6 9 12 15 18 9" />
                                        </svg>
                                    </span>
                                </div>
                                ...
                                <div class="relative">
                                    <input type="number" name="amount" value="{{ old('amount') }}"
                                        ...
                                        @class([
                                            ...
                                            'border-error-300 focus:border-error-300 focus:ring-error-500/10 dark:border-error-700' => $errors->has('amount'),
                                            'border-gray-300 focus:border-brand-300 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900' => !$errors->has('amount'),
                                        ])>
                                ...
                                <div class="relative">
                                    <input type="date" name="date" value="{{ old('date', now()->format('Y-m-d')) }}"
                                        @class([
                                            ...
                                            'border-error-300 focus:border-error-300 focus:ring-error-500/10 dark:border-error-700' => $errors->has('date'),
                                            'border-gray-300 focus:border-brand-300 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900' => !$errors->has('date'),
                                        ])>
                                    @if ($errors->has('date'))
                                        <span class="absolute top-1/2 right-3.5 -translate-y-1/2">
                                            @include('partials.error-icon')
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="description" rows="2" placeholder="e.g. Monthly rent payment, fuel for delivery"
                                    class="dark:bg-dark-900 shadow-theme-xs w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden focus:border-brand-300 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('description') }}</textarea>
                            </div>
                ...
                <div class="rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-white/[0.03] mb-5 overflow-hidden"
                    x-show="capitalAccountId && paymentType">
                    ...
                    <div class="divide-y divide-gray-100 dark:divide-gray-800">
                        <div class="flex items-center justify-between px-5 py-3">
                            <div class="flex items-center gap-2">
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-700 dark:bg-orange-500/10 dark:text-orange-400">DR</span>
                                <span class="text-sm text-gray-700 dark:text-gray-300">Capital/Deposit account</span>
                            </div>
                            <span class="text-sm text-gray-400 dark:text-gray-500">amount</span>
                        </div>
                        <div class="flex items-center justify-between px-5 py-3">
                            <div class="flex items-center gap-2">
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400">CR</span>
                ...
                {{-- Footer Actions --}}
                <div class="flex items-center justify-end gap-3 rounded-2xl border border-gray-200 bg-white px-5 py-4 sm:px-6 dark:border-gray-800 dark:bg-white/[0.03]">
                    <a href="{{ route('capitalDeposit.index') }}"
    ...
    @push('scripts')
        <script>
            function capitalDepositForm() {
                return {
                    capitalAccountId: '{{ old('account_id', '') }}',
